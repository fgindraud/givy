...
namespace Allocator {
	namespace Parts {
		...
		struct BumpPointerBase {
			...
			BumpPointerBase (Ptr start) : left_ (start), left_mapped_ (start), right_ (start), right_mapped_ (start) {
				ASSERT_OPT (start.is_aligned (VMem::PageSize));
			}
			~BumpPointerBase () { VMem::unmap_noexcept (left_mapped_, right_mapped_ - left_mapped_); }
		...
		template <typename Primary, typename Secondary> class Fallback : private Primary, private Secondary {
		public:
